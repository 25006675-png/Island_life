...
def petal_mesh():
    # One continuous silhouette: two round side lobes and a longer round tip.
    # The two notches remain visible even when leaves overlap.
    curves=[
        ((0,0),(.13,.07),(.44,.17),(.53,.34)),
        ((.53,.34),(.72,.64),(.48,.91),(.25,.72)),
        ((.25,.72),(.34,1.30),(-.34,1.30),(-.25,.72)),
        ((-.25,.72),(-.48,.91),(-.72,.64),(-.53,.34)),
        ((-.53,.34),(-.44,.17),(-.13,.07),(0,0)),
    ]
    outline=[]
    for controls in curves:
        for i in range(14):
            ...
            outline.append(p)
    def curved(x,y):
        return (x,y,.13*math.sin(math.pi*min(1,y))-.17*y*y-.13*x*x)
    ...
    data=bpy.data.meshes.new('Three rounded lobes • curved continuous leaf')
...
def crown(index, base, radius, height, rows):
    ...
    def place_leaf(t,a,fan,lift,length_factor,shade,skirt=False):
        ...
        if index==2 and t<.30:
            length*=.55+t*1.5
        obj.scale=(length*random.uniform(.64,.75),length,length)
