"""Ghibli purple tree, v6 - tier separation and tone match.

Run: blender --background --python create_ghibli_tree_v6.py
...
LEAF_SRGB = [
    (233, 202, 236), (219, 182, 231), (203, 163, 227), (189, 148, 222),
    (170, 137, 218), (150, 128, 213), (131, 119, 207), (118, 108, 190),
    (106,  97, 172), ( 96,  86, 150), ( 90,  78, 126), ( 96,  80, 102),
]
...
def crown(index, base, radius, height, exponent, anchors, rim_n,
    ...
    for k in range(anchors):
        ...
        exposure = .16 + .42 * lit + .26 * side + .22 * crest + random.uniform(-.11, .11)
    ...
    for tmin, tmax, drop, pitch_lo, pitch_hi, row in rim_rows:
        for k in range(rim_n):
            ...
            side = .5 + .5 * math.cos(a - KEY_AZIMUTH)
            exposure = (.40 + .28 * side + random.uniform(-.09, .09)) * (1 - .20 * row)
...
crown(0, base=2.86, radius=1.66, height=1.30, exponent=1.95, anchors=290, rim_n=48)
crown(1, base=4.78, radius=1.21, height=.86, exponent=1.95, anchors=185, rim_n=36)
crown(2, base=6.20, radius=.84, height=1.14, exponent=1.30, anchors=125, rim_n=26,
      core_drop=.02, core_shade=3, bare_tip=.15)
...
camera.rotation_euler = (Vector((0, 0, 3.72)) - camera.location).to_track_quat('-Z', 'Y').to_euler()
cam_data.type = 'ORTHO'
...
scene.render.filepath = str(ROOT / 'ghibli_purple_tree_v6.png')
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT / 'ghibli_purple_tree_v6.blend'))
