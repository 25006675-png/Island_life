...
|---|---|---|---|
| [Purple tree](Purple%20tree/) | `create_ghibli_tree_v6.py` (archived) | ~18 s | 854 leaves |
...
| [Coral palm](Coral%20palm/) | `create_palm.py` | ~21 s | 672 pinnae on 14 fronds, 114 trunk plates |
...
| Sage topiary (archived) | `create_topiary.py` | ~30 s | 2,316 leaves in 19 puffs, 4 mushrooms |
| [Magic mushrooms](Magic%20mushrooms/) | `create_magic_mushrooms.py` | ~30 s | 1 tree-sized bell-capped amanita on a root mound, 3 small ones at its foot, glowing gills and spots |
...
foot. Built for Errands, then replaced there by the magic mushrooms; now in
`archive/trees/Sage topiary/`.

**Magic mushrooms** (Errands) — one tree-sized amanita with a rounded bell
cap on a short, thick stem, and three small ones growing straight up at the
root mound's edge. Small stems leaning their feet into the mound bent sideways
and read as arms branching off it. The earlier builds (five, then three flat parasol caps on
tall stems) showed too little teal from the side next to the other species'
canopies. The bell's profile exponent is 2.2: at 1.5 it still read as a
straight cone. The gills are radial ridges painted into vertex
