...
An S-curved trunk clad in overlapping shield plates, carrying fourteen arching
feathered fronds in three whorls. The reference has eight; the crown was filled
out past it so the palm's coloured area reads as large in-game as the other
species' canopies (the trunk is as the reference: bbox 259x391 of 420, lower
~65% of the height).
...
LEAFLETS = [gs.blade('Pinna %d' % i, leaflet_outline(random.uniform(.14, .19)),
...
# Three whorls spread evenly around the crown - risers, spreaders, droopers -
# so it reads equally full from every heading. The first eight bunched their
# risers on one side, and the crown's in-game area swung 3x with the tree's yaw.
# az deg, elevation deg, length, tip rise relative to the crown
FRONDS = ([(18 + 72 * i + random.uniform(-8, 8), 64, 2.55, 1.55) for i in range(5)] +
          [(54 + 72 * i + random.uniform(-8, 8), 30, 2.85, -.05) for i in range(5)] +
          [(90 * i + random.uniform(-8, 8), 12, 2.60, -.80) for i in range(4)])
...
for fi, (az_d, el_d, L, tip_rise) in enumerate(FRONDS):
    ...
    for i in range(n):
        ...
        for sgn in (-1, 1):
            ...
            aim(o, fwd, up)
            ln = (1.18 * prof + .14) * random.uniform(.88, 1.12)
            o.scale = (ln, ln, ln)
            lit = up.dot(KEY3) * .5 + .5
...
# Viewed near eye level; framed wider than the reference to fit the fuller crown.
gs.studio(studio_g, cam_pos=(1.0, -20, 5.6), target_z=3.90, ortho_scale=9.60,
