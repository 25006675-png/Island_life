...
The Errands grove, replacing the sage topiary. One tree-sized amanita with a
tall rounded bell cap on a short, thick stem, and three small ones growing up
around its foot.
The first build had three flat parasol caps on tall stems: seen from the side
they were thin teal bands, too little colour next to the other species'
canopies. The bell is rounded, not a straight cone, which reads as a witch's
hat. Pale cap spots and radial gills glow like lanterns.
...
T_STEM = .20
UNDER_DEEP = .34          # how far the underside vaults up, x cap radius
CAP_P = 2.2               # cap profile: 1 is a straight cone (1.5 still read as one), 3.6 was a flat dome
FLARE = .07               # the bell's rim kicks out a little
...
def cap(name, cx, cy, rim_z, R, H):
    """Revolved bell: rounded tall top, concave gilled underside, flared rim."""
    verts, faces = [], []

    def radius_at(t, a):
        return R * t * (1 + FLARE * t ** 8 + .022 * math.sin(3 * a + cx)
                        + .013 * math.cos(5 * a - cy))
...
def cap_spots(name, cx, cy, rim_z, R, H, count):
    ...
    for n in range(count):
        ...
        r = R * t
        z = rim_z + H * (1 - t ** CAP_P)
        slope = math.atan(CAP_P * H / R * max(t, .04) ** (CAP_P - 1))
        nrm = Vector((math.sin(slope) * math.cos(a), math.sin(slope) * math.sin(a),
                      math.cos(slope)))
        rad = random.uniform(.10, .20) * (R / 1.26)        # fewer, bigger spots
...
# One big bell (cap height ~2/3 of its width) on a short, thick stem, and
# three small bells growing straight up from the ground at the root mound's
# edge, spread ~120 degrees apart so some show from any heading. Their feet
# tuck in a little so the caps lean out. Leaning their feet into the mound
# instead bent the stems sideways, like arms branching off it. Only the big
# one wears a skirt: on the thin stems it read as a knuckle. Stem tops sit a
# little up into each cap's vaulted underside.
# cx, cy, rim_z, R, H, stem_top, r_top, r_base, skirt_u, skirt_r, spots, lean
SHROOMS = [
    (0.00, .10, 2.55, 1.95, 2.55, 3.05, .52, .78, .60, .78, 18, (-.02, -.10)),
    (-1.27, -.89, 1.00, .50, .60, 1.13, .12, .19, 0, 0, 5, (.25, .17)),
    (1.30, -.75, .78, .375, .46, .88, .10, .15, 0, 0, 4, (-.22, .12)),
    (-.26, 1.48, .58, .275, .34, .65, .08, .12, 0, 0, 3, (.05, -.25)),
]
